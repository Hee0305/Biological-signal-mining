...
st.sidebar.header('Clevaeland Heart disease, clean data') 
st.sidebar.subheader('Visualization using seaborn and plotly by pdm06')
...
with row0_1: 
    hist_x = st.selectbox("Select a feature", options=df.columns, index=df.columns.get_loc("age")) #Pregnancies
...
with row2_1: 
    corr_x = st.selectbox("Correlation - X variable", options=df.columns, index=df.columns.get_loc("thalach"))
        
with row2_2: 
    corr_y = st.selectbox("Correlation - Y variable", options=df.columns, index=df.columns.get_loc("thal"))
...
correlations = df2.corr(method='pearson') # corr = 연관성
